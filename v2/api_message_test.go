...
func TestPostText(t *testing.T) {
	resp, err := bot.PostText(t.Context(), "PostText: email hello, world", WithEmail(testUserEmail))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
	resp, err = bot.PostText(t.Context(), "PostText: open_id hello, world", WithOpenID(testUserOpenID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
	resp, err = bot.PostText(t.Context(), "PostText: union_id hello, world", WithUnionID(testUserUnionID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
	resp, err = bot.PostText(t.Context(), "PostText: chat_id hello, world", WithChatID(testGroupChatID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
	resp, err = bot.PostText(t.Context(), "PostText: with formats <b>Bold</b> <i>italic</i> <u>Underline</u> <s>strikethrough</s> [Link](https://bytedance.com/)", WithChatID(testGroupChatID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
...
func TestPostTextMention(t *testing.T) {
	resp, err := bot.PostTextMention(t.Context(), "PostTextMention", testUserOpenID, WithChatID(testGroupChatID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
...
func TestPostTextMentionAll(t *testing.T) {
	resp, err := bot.PostTextMentionAll(t.Context(), "PostTextMentionAll", WithChatID(testGroupChatID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
...
func TestReplyMessage(t *testing.T) {
	resp, err := bot.PostText(t.Context(), "Message to be replied", WithChatID(testGroupChatID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
	replyID := resp.Data.MessageID
	resp, err = bot.PostTextMentionAndReply(t.Context(), "PostTextMentionAndReply", testUserOpenID, WithChatID(testGroupChatID), replyID)
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
	...
	resp, err = bot.ReplyMessage(t.Context(), om)
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
...
func TestReplyMessageFailed(t *testing.T) {
	...
	_, err := bot.ReplyMessage(t.Context(), om)
	assert.ErrorIs(t, err, ErrParamMessageID)
}

func TestPostImage(t *testing.T) {
	resp, err := bot.PostImage(t.Context(), "img_a97c1597-9c0a-47c1-9fb4-dd3e5e37ac9g", WithChatID(testGroupChatID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
...
func TestPostShareChat(t *testing.T) {
	resp, err := bot.PostShareChat(t.Context(), testGroupChatID, WithChatID(testGroupChatID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
...
func TestPostShareUser(t *testing.T) {
	resp, err := bot.PostShareUser(t.Context(), testUserOpenID, WithChatID(testGroupChatID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
...
func TestPostFailedByUserID(t *testing.T) {
	_, err := bot.PostText(t.Context(), "should fail", &OptionalUserID{"some id", ""})
	assert.ErrorIs(t, err, ErrParamUserID)
	_, err = bot.PostTextMention(t.Context(), "should fail", "", &OptionalUserID{"some id", ""})
	assert.ErrorIs(t, err, ErrParamUserID)
	_, err = bot.PostTextMentionAll(t.Context(), "should fail", &OptionalUserID{"some id", ""})
	assert.ErrorIs(t, err, ErrParamUserID)
	_, err = bot.PostTextMentionAndReply(t.Context(), "should fail", "", &OptionalUserID{"some id", ""}, "")
	assert.ErrorIs(t, err, ErrParamUserID)
	_, err = bot.PostImage(t.Context(), "should fail", &OptionalUserID{"some id", ""})
	assert.ErrorIs(t, err, ErrParamUserID)
	_, err = bot.PostShareChat(t.Context(), "should fail", &OptionalUserID{"some id", ""})
	assert.ErrorIs(t, err, ErrParamUserID)
	_, err = bot.PostShareUser(t.Context(), "should fail", &OptionalUserID{"some id", ""})
	assert.ErrorIs(t, err, ErrParamUserID)
}
...
func TestPostMessage(t *testing.T) {
	...
	resp, err := bot.PostMessage(t.Context(), om)
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}
...
func TestPostPostMessage(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		newOM := NewMsgBuffer(MsgPost).
			...
			Build()
		_, err = bot.UpdateMessage(t.Context(), resp.Data.MessageID, newOM)
		if assert.NoError(t, err) {
			assert.Equal(t, 0, resp.Code)
		}
	}
...
func TestPostCardMessage(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		newOM := msg.BindEmail(testUserEmail).Card(newCard.String()).Build()
		resp, err := bot.UpdateMessage(t.Context(), resp.Data.MessageID, newOM)
		t.Log(err, resp)
	}
}
...
func TestTemplateCardMessage(t *testing.T) {
	...
	msg := NewMsgBuffer(MsgInteractive)
	om := msg.BindEmail(testUserEmail).Template(c).Build()
	resp, err := bot.PostMessage(t.Context(), om)
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
...
func TestEphemeralMessage(t *testing.T) {
	...
	resp, err := bot.PostEphemeralMessage(t.Context(), om)
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
	}

	delResp, err := bot.DeleteEphemeralMessage(t.Context(), resp.Data.MessageID)
	if assert.NoError(t, err) {
...
func TestMessageCRUD(t *testing.T) {
	// create
	resp, err := bot.PostText(t.Context(), "PostText: CRUD hello, world", WithChatID(testGroupChatID))
	if assert.NoError(t, err) {
		assert.Equal(t, 0, resp.Code)
		assert.NotEmpty(t, resp.Data.MessageID)
		t.Log(resp.Data)
	...
	// get
	getResp, err := bot.GetMessage(t.Context(), resp.Data.MessageID)
	if assert.NoError(t, err) {
	...
	// recall
	rcResp, err := bot.RecallMessage(t.Context(), resp.Data.MessageID)
	if assert.NoError(t, err) {
		assert.Equal(t, 0, rcResp.Code)
	}
	// receipt unread
	receipt, err := bot.MessageReadReceipt(t.Context(), resp.Data.MessageID)
	if assert.NoError(t, err) {
...
func TestIdempotentMessage(t *testing.T) {
	...
	_, _ = bot.PostMessage(t.Context(), om)
...
func TestPinMessages(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		resp, err := bot.PinMessage(t.Context(), messageID)
		if assert.NoError(t, err) {
			assert.Equal(t, 0, resp.Code)
			assert.Equal(t, messageID, resp.Data.Pin.MessageID)
			unpinResp, err := bot.UnpinMessage(t.Context(), messageID)
...
func TestMessageReactions(t *testing.T) {
	...
	if assert.NoError(t, err) {
		messageID := resp.Data.MessageID
		resp, err := bot.AddReaction(t.Context(), messageID, EmojiTypeOK)
		if assert.NoError(t, err) {
			assert.Equal(t, 0, resp.Code)
			assert.Equal(t, EmojiTypeOK, resp.Data.ReactionType.EmojiType)
			deleteReactionResp, err := bot.DeleteReaction(t.Context(), messageID, resp.Data.ReactionID)
			assert.NoError(t, err)
...
func TestForwardMessage(t *testing.T) {
	...
	if assert.NoError(t, err) {
		messageID := resp.Data.MessageID
		resp, err := bot.ForwardMessage(t.Context(), messageID, WithChatID(testGroupChatID))
		if assert.NoError(t, err) {
			assert.Equal(t, 0, resp.Code)
		}
	}
