...
go-lark v2 triggers auto-renewable retrieval of tenant access token by default.
You may call any API you like.
...
If a HTTP error happend, `err` would be the error returned by HTTP client. Otherwise, if it was not `nil`, it would be error wrapping Lark API error.
